...
- **Text preprocessing**: The script loads rules from `speech_prep_prompt.md`, calls `gpt-5-mini` with an enforced JSON schema, and consumes the `normalized_text` field. Pass `-n/--no-normalize` to bypass this step and send the raw text straight to speech synthesis.
